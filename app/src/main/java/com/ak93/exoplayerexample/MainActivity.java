...
//        String videoPath = Environment.getExternalStorageDirectory()+"/Video/TehzeebHafi.mp4";
//        localVideo = Uri.parse(videoPath);
        videosList = new ArrayList<>();
        videoPathsList = getAllMedia();
//        prepareExoPlayerFromFileUri(localVideo);
        prepareExoPlayerList();
    ...
    private void prepareExoPlayerList(){
        exoPlayer = ExoPlayerFactory.newSimpleInstance(this, new DefaultTrackSelector(null), new DefaultLoadControl());
        exoPlayer.addListener(eventListener);

        for(String singlePath: videoPathsList){
            Uri uri = Uri.parse(singlePath);
            DataSpec dataSpec = new DataSpec(uri);
            final FileDataSource fileDataSource = new FileDataSource();
            try {
                fileDataSource.open(dataSpec);
            ...
            MediaSource audioSource = new ExtractorMediaSource(fileDataSource.getUri(),
                    factory, new DefaultExtractorsFactory(), null, null);
            videosList.add(audioSource);
        }
        mediaSources = videosList.toArray(new MediaSource[videosList.size()]);
        ConcatenatingMediaSource videosSource = new ConcatenatingMediaSource(mediaSources);


        exoPlayer.prepare(videosSource);
        initMediaControls();
    }
